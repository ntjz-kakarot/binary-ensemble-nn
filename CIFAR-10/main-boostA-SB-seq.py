...
parser.add_argument('--lr', action='store', default=0.01, type=float, help='the intial learning rate')
...
if not os.path.isfile(args.data + '/train_data.npy'):
...
def test(save_name, best_acc, sample_weights=torch.Tensor(np.ones((50000,1))/50000.0)):
    ...
    with torch.no_grad():
        for items in testloader_in:
            if len(items) == 3:
                data, target, weight = items
                # If weight is used in the logic, apply it here.
            else:
                data, target = items
